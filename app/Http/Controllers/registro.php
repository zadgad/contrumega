...
class registro extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $this->validate(
            ...
            $prueba=DB::table('usr')->where('ci_per','=',$ci)->count();
            $prueba1=DB::table('usr')->where('usr.email','=',$email)->count();

            //  $prueba=DB::table('usr')->select(DB::raw('count(*) as usr_count, ci_per'))->where('ci_per','=',$ci);
            ...
            if ($prueba<1) {
                if ($prueba1<1) {
                 if($password==$passwordR){
                     ...
                     //$usr = usr::where('ci_per','=', $ci)->select('id_usr')->get();
                     $getuser = DB::table('usr')->where('ci_per', '=', $ci)->select('id_usr', 'email', 'password')->get();
                     //return $insertpersona." user".$insertuser."get ".$getuser;

                     $id=DB::table('usr')->where('ci_per','=',$ci )->pluck('id_usr');
